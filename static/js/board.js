function handleSubmit() {
  ...
  var selectedCity = document.getElementById('citySelect');
  var selectedDistrict = document.getElementById('districtSelect');
  ...
  if (selectedCity && selectedDistrict && (selectedCity.value.trim() == '' || selectedDistrict.value.trim() == '')) {
      alert('지역과 구군을 모두 선택해주세요.'); 
      return false;
  }
...
function handleChange(selectElement) {
  ...
  if (selectedValue === 'location') {
      ...
      inputGroupElement.className = 'input-group row';

      var selectElement1 = document.createElement('select');
      selectElement1.name = 'city';
      selectElement1.className = 'form-select col-sm-6';
      selectElement1.style.marginRight = '5px';
      selectElement1.id = 'citySelect';

      var option1 = document.createElement('option');
      option1.value = '';
      option1.textContent = '지역을 선택하세요';
      selectElement1.appendChild(option1);

      for (var city in jsonData.cities) {
      ...
      selectElement2.name = 'district';
      selectElement2.className = 'form-select col-sm-6';
      selectElement2.id = 'districtSelect';

      var option2 = document.createElement('option');
      option2.value = '';
      option2.textContent = '구군을 선택하세요';
      selectElement2.appendChild(option2);

      selectElement1.addEventListener('change', loadDistricts);

      function loadDistricts() {
        var selectedCity = selectElement1.value;

        selectElement2.innerHTML = '<option value="">구군을 선택하세요</option>';

        if (selectedCity) {
          var districts = jsonData.cities[selectedCity];
      ...
      submitButton.className = 'btn btn-primary col-sm-2';
      submitButton.textContent = '검색';

      inputGroupElement.appendChild(selectElement1);
      inputGroupElement.appendChild(selectElement2);
      inputGroupElement.appendChild(submitButton);

      searchInputContainer.appendChild(inputGroupElement);
    ...
    inputElement.className = 'form-control col-sm-6';
    inputElement.placeholder = '검색어를 입력하세요';
